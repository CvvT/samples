...
CC			= clang #/usr/i386-linux-cgc/bin/clang
LD			= ld #/usr/i386-linux-cgc/bin/ld
CXX			= clang++ # /usr/i386-linux-cgc/bin/clang++
OBJCOPY			= objcopy #/usr/i386-linux-cgc/bin/objcopy
LD_ELF                  = /usr/bin/ld
...
CGC_CFLAGS	= -nostdlib -fno-builtin -nostdinc -Iinclude -Ilib -I/usr/include $(CFLAGS) -DCGC_BIN_COUNT=$(BIN_COUNT) -m32
#else
...
QEMU_CGC		= --wrapper /vagrant/cgc-afl/afl-1.86b/afl-qemu-trace
...
get_cb_bins   		= $(sort $(filter-out %_partial, $(filter-out %_patched, $(notdir $(wildcard $(BIN_DIR)/$(EXE)*))))) $(QEMU_CGC)
get_cb_patched_bins 	= $(sort $(filter-out %_partial, $(filter %_patched, $(notdir $(wildcard $(BIN_DIR)/$(EXE)*))))) $(QEMU_CGC)
...
$(PATCHED_ELF_STUB):
	# CHANGED : add -m32
	$(CC) -m32 -c -nostdlib -fno-builtin -nostdinc -o $(BUILD_DIR)/$(PATCHED_DIR)/syscall-stub.elf /usr/share/cb-testing/syscall-stub.c
...
$(PATCHED_SO): $(PATCHED_ELF_OBJS) $(PATCHED_ELF_STUB)
	# CHANGED : add elf_i386
	# $(LD_ELF) -melf_i386 -shared -o $@ $^
	$(LD_ELF) -shared -o $@ $^ 
endif
